...
val () =
  (err := err + 1)
val () =
  the_staerrlst_add(STAERR_s2Var_s2exp_solve(loc0, s2V1, s2e2))
//
...
(
  loc: location
, lbs: s2VarBoundlst, s2f: s2hnf, err: &int
...
(
  loc: location
, s2f: s2hnf, ubs: s2VarBoundlst, err: &int
...
s2e2 = s2hnf2exp (s2f2)
val () =
s2Var_s2exp_srtck_err(loc0, s2V1, s2e2, err)
//
val
isimp = s2exp_is_impred(s2e2)
//
val () =
if isimp then
{
//
  val
  s2ze2 = s2zexp_make_s2exp (s2e2)
//
  val () =
  s2Var_merge_szexp_err(loc0, s2V1, s2ze2, err)
//
} // end of [if] // end of [val]
//
val () =
  s2Var_set_link(s2V1, Some s2e2)
//
val () =
if isimp then
{
...
in
  s2hnf_tyleq_solve_ubs_err(loc0, s2f2, ubs, err)
end // end of [val]
//
} // end of [if] // end of [val]
//
...
s2e1 = s2hnf2exp(s2f1)
val () =
s2Var_s2exp_srtck_err(loc0, s2V2, s2e1, err)
//
val
isimp = s2exp_is_impred(s2e1)
//
val () =
if isimp then
{
...
in
  s2hnf_tyleq_solve_lbs_err(loc0, lbs, s2f1, err)
end // end of [val]
val () = let
...
var err: int = 0
val ((*void*)) =
  s2eff_subeq_solve_err(loc0, s2fe1, s2fe2, err)
//
...
| (_, S2EFFexp (s2e2)) => (
  ...
  | S2EVar s2V2 => let
      val s2e1 = s2exp_eff0 (s2fe1)
      val s2f1 = s2exp2hnf_cast (s2e1)
      val s2f2 = s2exp2hnf_cast (s2e2)
    in
      s2hnf_equal_solve_rVar_err
        (loc0, s2f1, s2f2, s2V2, err)
      // end of [s2hnf_equal_solve_rVar_err]
    end // end of [S2EVar]
  | _ => let
      val () = err := err + 1
    in
      the_staerrlst_add
        (STAERR_s2eff_subeq(loc0, s2fe1, s2fe2))
      // the_staerrlst_add
    end // end of [_]
  ) // end of [S2EFFexp]
| (_, _) => let
...
//
val ((*void*)) =
s2exp_equal_solve_err(loc0, s2e10, s2e20, err)
//
...
| (_(*non-S2Ecstapp*), _(*non-S2Ecstapp*)) => false
//
...
//
if absconeq
  then aux_solve(loc0, s2e1, s2e2, err) else (err := err + 1)
//
...
| (S2EVar s2V1, _) =>
  ...
  | S2EVar s2V2
    when s2V1 = s2V2 => ()
  | _ (* non-S2EVar *) =>
    s2hnf_equal_solve_lVar_err(loc0, s2f10, s2f20, s2V1, err)
  ) // end of [S2EVar, _]
| (_, S2EVar s2V2) =>
  (
...
| (S2Eat(s2e11, s2e12), _) =>
  ...
  | S2Eat(s2e21, s2e22) => let
      val () =
        s2exp_equal_solve_err(loc0, s2e11, s2e21, err)
      val () =
        s2exp_equal_solve_err(loc0, s2e12, s2e22, err)
    in
      // nothing
    end // end of [S2Eat]
  | _ (*non-S2Eat*) => (err := err + 1)
  )
//
...
then
the_staerrlst_add
  (STAERR_s2exp_equal(loc0, s2e10, s2e20))
// end of [if] // end of [val]
in
  // nothing
...
| (_, S2Einvar s2e21) => let
  ...
  in
    ...
    | S2EVar s2V2 =>
      s2hnf_equal_solve_rVar_err
        (loc0, s2f10, s2f21, s2V2, err)
      // s2hnf_equal_solve_rVar_err
    | _ (*non-S2Evar*) =>
      s2hnf_tyleq_solve_err(loc0, s2f10, s2f21, err)
  end // end of [_, S2Einvar]
//
| (_, S2EVar s2V2) =>
...
*)
      in
        ...
        then let
          val-
          list_cons(argsrts, _) =
          s2cst_get_argsrtss(s2c1)
        in
          s2explst_tyleq_solve_argsrtlst_err
            (loc0, argsrts, s2es1_arg, s2es2_arg, err)
          // s2explst_tyleq_solve_argsrtlst_err
        end // end of [then]
        else (err := err + 1) // end of [else]
...
| (S2Eat(s2e11, s2e12), _) =>
  ...
  | S2Eat(s2e21, s2e22) => let
      val () =
        s2exp_tyleq_solve_err(loc0, s2e11, s2e21, err)
      val () =
        s2exp_equal_solve_err(loc0, s2e12, s2e22, err)
    in
      // nothing
...
//
| (_, _) when
  s2hnf_syneq2(s2f10, s2f20) => ((*sound-but...*))
//
| (_(*rest-of-s2exp*), _(*rest-of-s2exp*)) => (err := err + 1)
//
...
//
  val () = 
  the_staerrlst_add
  (STAERR_s2exp_tyleq(loc0, s2e10, s2e20))
//
} (* end of [if] *)
//
in
...
// HX: shape mismatch:
//
| (_(*rest-wths2exp*), _(*rest-wths2exp*)) =>
  let val ((*void*)) = err := err + 1 in (1) end
//
end // end of [loop]
//
val
...
//
      val
      s2f_lb =
      s2exp2hnf(s2VarBound_get_val(lb))
//
      val () =
      s2hnf_tyleq_solve_err(loc0, s2f_lb, s2f, err)
//
    in
      s2hnf_tyleq_solve_lbs_err(loc0, lbs, s2f, err)
    end // end of [list_cons]
...
(*
macdef
loop = s2hnf_tyleq_solve_ubs_err
*)
//
in
  case+ ubs of
...
//
    ...
    in
      s2hnf_tyleq_solve_ubs_err(loc0, s2f, ubs, err)
    end // end of [list_cons]
...
(
println!
  ("s2exp_hypequal_solve_abscon: s2f1 = ", s2f1);
println!
  ("s2exp_hypequal_solve_abscon: s2f2 = ", s2f2);
) (* end of [val] *)
*)
val s2e1 = s2hnf2exp(s2f1)
and s2e2 = s2hnf2exp(s2f2)
...
//
| (S2Eapp(s2e1, _),
   S2Eapp(s2e2, _)) => auxcheck(s2e1, s2e2)
//
| (_(*non-S2Ecstapp*), _(*non-S2Ecstapp*)) => false
)
in
...
val () =
println!
  ("s2explst_hypequal_solve: s2es1 = ", s2es1)
val () = 
println!
  ("s2explst_hypequal_solve: s2es2 = ", s2es2)
*)
//
in
//
