...
| _ when name = $SYM.symbol_TBOX => $SYM.symbol_VTBOX
| _ when name = $SYM.symbol_TFLT => $SYM.symbol_VTFLT
| _ => name
//
...
*)
      else if s2cst_get_iscon (s2c) then true else false
    ) // end of [S2Ecst]
...
| S2Eeff (s2fe) => let
  ...
  in
    if flag > f0 then s2exp_eff0 (s2fe) else s2e0
  end // end of [S2Eeff]
